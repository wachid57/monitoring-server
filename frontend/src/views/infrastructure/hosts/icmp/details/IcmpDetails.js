...
const IcmpDetails = () => {
  ...
  // alerts hook (reuse monitoring implementation)
  ...
  useEffect(()=>{
    if(!icmpService?.id) return;
    let active = true;
    const fetchRealtime = async () => {
      try {
        const lastRes = await fetch(`${BACKEND_URL}${API_PREFIX}/infrastructure/hosts/${hostId}/icmp/${icmpService.id}/last`, { headers: getAuthHeaders() });
        if (lastRes.ok) {
          const txt = await lastRes.text();
          try { const obj = JSON.parse(txt); if(active && obj){ setMetrics(m=>({...m, currentPing: obj.latency_ms != null ? obj.latency_ms.toFixed(2): m.currentPing })); } } catch(e){}
        }
        const seriesRes = await fetch(`${BACKEND_URL}${API_PREFIX}/infrastructure/hosts/${hostId}/icmp/${icmpService.id}/series?limit=200`, { headers: getAuthHeaders() });
        if (seriesRes.ok) {
          const arr = await seriesRes.json();
          // arr is array of JSON strings
          const samples = arr.map(s=>{ try { return JSON.parse(s);} catch(e){ return null; } }).filter(Boolean);
          const okLat = samples.filter(s=> s.status==='OK' && typeof s.latency_ms==='number').map(s=> s.latency_ms);
          if(active && okLat.length>0){
            const avg = okLat.reduce((a,b)=>a+b,0)/okLat.length;
            setMetrics(m=>({...m, avgPing24h: avg.toFixed(2) }));
          }
        }
      } catch(e){ /* silent */ }
    };
    fetchRealtime();
    const id = setInterval(fetchRealtime, 30000); // 30s
