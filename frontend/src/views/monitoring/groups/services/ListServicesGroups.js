...
const ListHosts = () => {
  ...
  const fetchHosts = async () => {
    ...
    setError('');
    try {
  const res = await fetch(BACKEND_URL + API_PREFIX + '/monitoring/groups/services', { headers: getAuthHeaders() });
  ...
  const handleDelete = async (hostId) => {
    try {
  const res = await fetch(BACKEND_URL + API_PREFIX + `/monitoring/groups/services/${hostId}`, {
        method: 'DELETE',
          ...
          <Button variant="contained" color="primary" onClick={async () => {
            ...
            try {
              ...
              if (editMode && editingHostId) {
                res = await fetch(BACKEND_URL + API_PREFIX + `/monitoring/groups/services/${editingHostId}`, { method: 'PUT', headers: {'Content-Type': 'application/json', ...getAuthHeaders()}, body: JSON.stringify(newHost) });
              } else {
                res = await fetch(BACKEND_URL + API_PREFIX + '/monitoring/groups/services', { method: 'POST', headers: {'Content-Type': 'application/json', ...getAuthHeaders()}, body: JSON.stringify(newHost) });
              }
              if (res.status === 401 || res.status === 403) return handleAuthError({ status: res.status });
              const data = await res.json();
              if (res.ok) {
