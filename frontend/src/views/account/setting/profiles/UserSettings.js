...
export default function UserSettings() {
	...
	const saveRow = async (row) => {
		await fetch(BACKEND_URL + API_PREFIX + '/account/setting/profiles/user-settings', {
			method: 'POST',
	...
	const removeRow = async (key) => {
		...
		try {
			const res = await fetch(BACKEND_URL + API_PREFIX + '/account/setting/profiles/user-settings/' + key, { 
				method: 'DELETE',
				headers: getAuthHeaders()
			});
			if (res.status === 401 || res.status === 403) { handleAuthError({ status: res.status }); return; }
